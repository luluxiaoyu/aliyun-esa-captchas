  ...
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/get_ticket" && request.method === "POST") {
      ...
      if (verifyCode === "T001") {
        ...
        return responseJSON(0, "验证通过", { ticket: ticket, expire_in: EXPIRE });
      } else {
        let errorMsg = "验证失败";
        if (verifyCode === "F019") errorMsg = "验证超时或未验证";
        if (verifyCode === "F020") errorMsg = "票据不匹配";
        return responseJSON(400, "验证码校验失败: " + errorMsg, { reason_code: verifyCode || "MISSING_HEADER" });
      }
    }
    ...
    // --- 接口: 验签 ---
      ...
      try {
        ...
        if (payload) {
          const now = Math.floor(Date.now() / 1000);
          if (payload.exp < now) return responseJSON(401, "验证失败：Ticket 已过期");
          if (payload.ip !== userIpFromBackend) return responseJSON(403, "安全警告：IP 地址不匹配", { registered_ip: payload.ip, current_ip: userIpFromBackend });
          return responseJSON(0, "Ticket 有效", { req_id: payload.req_id, verified_ip: payload.ip });
        } else {
